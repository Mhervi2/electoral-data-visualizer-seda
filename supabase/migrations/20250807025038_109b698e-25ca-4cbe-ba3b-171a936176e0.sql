...
-- Add constraint using a different approach for IDM range validation
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_idm_reasonable_range'
    ) THEN
        ALTER TABLE public.mpca 
        ADD CONSTRAINT chk_idm_reasonable_range 
