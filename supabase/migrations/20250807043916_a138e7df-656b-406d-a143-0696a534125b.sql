-- Fix audit log foreign key constraint by using valid UUID for automated imports
-- First, let's get or create a system user UUID for automated operations

-- Update the audit functions to use a valid approach for changed_by
...
DECLARE
  ...
  system_user_id UUID;
BEGIN
  -- Get system user ID (superadmin) for automated imports
  SELECT id INTO system_user_id 
  FROM auth.users 
  WHERE email = 'superadmin@seda.es' 
  LIMIT 1;
  
  -- Use auth.uid() if available, otherwise use system user, fallback to null
  change_user := COALESCE(auth.uid(), system_user_id);
  
  -- Only proceed if we have a valid user ID
  IF change_user IS NULL THEN
    RAISE EXCEPTION 'No valid user ID available for audit log';
  END IF;
...
BEGIN
  ...
  -- Use auth.uid() if available, otherwise use system user
  change_user := COALESCE(auth.uid(), system_user_id);
